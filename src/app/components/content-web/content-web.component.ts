...
@Component({
  ...
  standalone: true,
  imports: [HomeComponent, AboutComponent, ProjectComponent, SkillComponent, ContactComponent, CertificateComponent, ExperienceComponent, ContentCanvasComponent],
...
export class ContentWebComponent {
  ...
  onScroll(event: Event): void {
    ...
    let nav_menu_smart = document.getElementById('nav-menu-smart');
    ...
    /**
     * Propiedades Nav Menu SmartPhones
     */
    if (scrollY > 5) {
      nav_menu_smart?.classList.add('bg-opacity-90');   
      nav_menu_smart?.classList.remove('bg-opacity-0');   
    } else if(scrollY < 5){
      nav_menu_smart?.classList.remove('bg-opacity-90');
      nav_menu_smart?.classList.add('bg-opacity-0');
    }
